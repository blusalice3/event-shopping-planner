...
const ShoppingList: React.FC<ShoppingListProps> = ({
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (dragItem.current !== null) {
      // 異なる列間の移動
      if (dragSourceColumn.current && dragSourceColumn.current !== columnType) {
  ...
  const handleColumnDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (dragItem.current !== null && dragSourceColumn.current !== columnType) {
      const draggedIds = selectedItemIds.has(dragItem.current) 
        ? Array.from(selectedItemIds) 
        : [dragItem.current];
